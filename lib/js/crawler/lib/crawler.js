...
import { map, reduce, pick, isEmpty, uniq } from 'lodash-es';
import { KnownDevices as devices } from 'puppeteer';
...
export default class Crawler {
   ...
   */
  _handlePageEvents() {
    this._page.on('pageerror', (text) => Helper.debugConsole(text));
    this._page.on('console', (msg) =>
      Helper.debugConsole(`${msg.type()} ${msg.text()} at ${this._options.url}`)
    );
  ...
  async _handleDialog(dialog) {
    Helper.debugDialog(
      `${dialog.type()} ${dialog.message()} at ${this._options.url}`
    );
  ...
  async _waitFor() {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, this._options.waitFor.selectorOrFunctionOrTimeout)
    );
  ...
  async _collectLinks(baseUrl) {
    ...
    await this._page.exposeFunction('pushToLinks', (link) => {
      const _link = Helper.resolveUrl(link, baseUrl);
