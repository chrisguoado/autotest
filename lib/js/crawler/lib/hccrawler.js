...
const LAUNCH_OPTIONS = CONNECT_OPTIONS.concat([
  // 'ignoreHTTPSErrors',
  'headless',
  'executablePath',
  // 'slowMo',
...
export default class HCCrawler extends EventEmitter {
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    try {
      ...
      // crawler.close() may throw an error if the url is something unexpected,
      // ignore the error
      await crawler.close().catch((e) => e);
